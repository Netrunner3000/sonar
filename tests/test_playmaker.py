...
def test_nfl_is_registered():
    assert playmaker.get_sport("nfl").name == "NFL"
    assert playmaker.list_sports()


def test_unknown_sport_is_refused():
    with pytest.raises(ValueError):
        playmaker.get_sport("quidditch")
...
def test_american_to_decimal(odds, decimal):
    assert playmaker.american_to_decimal(odds) == pytest.approx(decimal, abs=1e-4)


def test_even_money_is_a_coin_flip():
    assert playmaker.implied_probability(100) == pytest.approx(0.5)


def test_standard_juice_implies_more_than_half():
    """-110 needs 52.38% to break even — the 2.38% over a coin flip is the vig."""
    assert playmaker.implied_probability(-110) == pytest.approx(0.5238, abs=1e-4)


def test_zero_odds_is_rejected():
    with pytest.raises(ValueError):
        playmaker.american_to_decimal(0)
...
def test_removing_vig_makes_two_sides_sum_to_one():
    a, b = playmaker.remove_vig(-110, -110)
    assert a + b == pytest.approx(1.0)
    assert a == pytest.approx(0.5)


def test_vig_removal_keeps_the_favourite_favoured():
    fav, dog = playmaker.remove_vig(-200, +170)
    assert fav > dog
...
def test_break_even_probability_has_no_expected_value():
    assert playmaker.expected_value(playmaker.implied_probability(-110), -110) == pytest.approx(0.0, abs=1e-9)


def test_edge_over_the_price_is_positive_ev():
    assert playmaker.expected_value(0.60, -110) > 0


def test_worse_than_the_price_is_negative_ev():
    assert playmaker.expected_value(0.45, -110) < 0


def test_expected_value_scales_with_stake():
    single = playmaker.expected_value(0.60, +100, stake=1.0)
    double = playmaker.expected_value(0.60, +100, stake=2.0)
    assert double == pytest.approx(single * 2)


def test_certain_win_returns_the_full_profit():
    assert playmaker.expected_value(1.0, +150, stake=10.0) == pytest.approx(15.0)


def test_certain_loss_returns_the_stake():
    assert playmaker.expected_value(0.0, +150, stake=10.0) == pytest.approx(-10.0)


@pytest.mark.parametrize("bad", [-0.1, 1.1])
def test_probability_outside_zero_to_one_is_rejected(bad):
    with pytest.raises(ValueError):
        playmaker.expected_value(bad, -110)
...
def test_no_edge_means_no_stake():
    assert playmaker.kelly_fraction(playmaker.implied_probability(-110), -110) == pytest.approx(0.0, abs=1e-9)


def test_negative_edge_never_suggests_a_stake():
    """A losing bet must return 0, not a negative fraction to 'lay' the other side."""
    assert playmaker.kelly_fraction(0.30, -110) == 0.0


def test_kelly_grows_with_the_edge():
    assert playmaker.kelly_fraction(0.70, +100) > playmaker.kelly_fraction(0.55, +100)


def test_kelly_at_even_money_is_twice_the_edge():
    """The textbook case: at +100, f = 2p - 1."""
    assert playmaker.kelly_fraction(0.60, +100) == pytest.approx(0.20)
...
def test_edge_is_measured_against_the_price_not_against_half():
    assert playmaker.edge_versus_market(0.5238, -110) == pytest.approx(0.0, abs=1e-4)
    assert playmaker.edge_versus_market(0.60, -110) > 0
...
def test_prompt_carries_every_supplied_value():
    prompt = playmaker.build_prompt(
        playmaker.NFL, "J. Allen", "Passing yards", "Over 252.5", "-110",
...
def test_prompt_marks_missing_inputs_rather_than_inventing_them():
    prompt = playmaker.build_prompt(playmaker.NFL, "", "Spread", "", "", "", "")
...
def test_sections_are_separated():
    result = playmaker.parse_analysis(SAMPLE)
...
def test_extracted_probability_feeds_expected_value():
    ...
    result = playmaker.parse_analysis(SAMPLE)
    assert playmaker.expected_value(result.win_probability, -110) > 0


def test_markdown_headings_are_tolerated():
    result = playmaker.parse_analysis(SAMPLE.replace("OVER CASE", "**OVER CASE**"))
    assert "Averaged 288" in result.sections["OVER CASE"]


def test_no_edge_is_read_as_a_verdict():
    result = playmaker.parse_analysis("EDGE ASSESSMENT\nLean: NO EDGE\nConfidence: low\n")
    assert result.lean == "NO EDGE"


def test_unparseable_text_does_not_raise():
    result = playmaker.parse_analysis("the model rambled without headings")
    assert result.lean == ""
